...
class Maze(object):
    ...
    def text_maze(self):
        ...
        for y in range(self.height-1, -1, -1):
            for x in range(self.width-1, -1, -1):
                if self.maze[x][y] == 0:
                    self.worldString = self.worldString + " "
                elif self.maze[x][y] == 1:
                    self.worldString = self.worldString + "#"
                else:
                    self.worldString = self.worldString + self.maze[x][y].strip(" ")
            self.worldString = self.worldString + "\n"
        return self.worldString.replace("*", "v").replace("@", "b")

def create_maze(width = 35, height = 21):
    m = Maze(width, height).text_maze().strip("\n").split("\n")
    for i in range(len(m)):
        m[i] = list(m[i])
    try:
        m[0][1] = 'a'
    except:
        raise Exception("Inappropriate maze dimensions")
    return m
